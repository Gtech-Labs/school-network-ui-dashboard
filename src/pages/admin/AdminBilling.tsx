...
import { Search, TrendingUp, AlertCircle, CheckCircle, FileText, Banknote, ChevronLeft, ChevronRight } from 'lucide-react';
...
const SUBSCRIPTION_RATES = { Trial: 50, Basic: 100, Premium: 150, Enterprise: 200 };
...
export default function AdminBilling() {
  ...
  const invoiceAmount = selectedSchool ? selectedSchool.studentsCount * SUBSCRIPTION_RATES[selectedSchool.plan] : 0;
  ...
  const pendingAmount = mockPayments.filter((p) => p.status === 'Pending').reduce((acc, p) => acc + p.amount, 0);
  const overdueAmount = mockPayments.filter((p) => p.status === 'Overdue').reduce((acc, p) => acc + p.amount, 0);

  const handleSendReminder = (schoolName: string) => {
    toast({ title: t('admin.billing.reminderSent'), description: `${t('admin.billing.paymentReminderSent')} ${schoolName}` });
  };

  const handleGenerateInvoice = () => {
    if (!selectedSchool) {
      toast({ title: 'Error', description: t('admin.billing.selectSchool'), variant: 'destructive' });
      return;
    }
    toast({ title: t('admin.billing.invoiceGenerated'), description: `Invoice of ${formatCurrency(invoiceAmount)} generated for ${selectedSchool.name}` });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button className="shadow-md" onClick={() => setIsInvoiceDialogOpen(true)}>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{mockPayments.filter((p) => p.status === 'Paid').length} {t('admin.billing.payments')}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{mockPayments.filter((p) => p.status === 'Pending').length} {t('admin.billing.invoices')}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{mockPayments.filter((p) => p.status === 'Overdue').length} {t('admin.billing.overdue').toLowerCase()}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader>
          ...
          <div className="mt-4 flex gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder={t('admin.billing.searchBySchool')} value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="pl-10" />
            </div>
            ...
            <TableBody>
              {paginatedPayments.map((payment) => (
                <TableRow key={payment.id}>
                  ...
                  <TableCell>{payment.paidDate ? payment.paidDate : '-'}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    {payment.status !== 'Paid' && (
                      <Button variant="outline" size="sm" onClick={() => handleSendReminder(payment.schoolName!)}>
      ...
      <Dialog open={isInvoiceDialogOpen} onOpenChange={setIsInvoiceDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>{t('admin.billing.subtitle')}</DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              <Label htmlFor="school">{t('admin.billing.selectSchool')}</Label>
              <Select value={selectedSchoolId} onValueChange={setSelectedSchoolId}>
                <SelectTrigger id="school"><SelectValue placeholder={t('admin.billing.chooseSchool')} /></SelectTrigger>
                <SelectContent>
            ...
            {selectedSchool && (
              <div className="space-y-4 rounded-lg border border-border bg-muted/50 p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{formatCurrency(SUBSCRIPTION_RATES[selectedSchool.plan])}</span>
                </div>
                <div className="h-px bg-border" />
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t('admin.billing.calculation')}</span>
                  <span className="text-sm text-muted-foreground">{selectedSchool.studentsCount} × {formatCurrency(SUBSCRIPTION_RATES[selectedSchool.plan])}</span>
                </div>
                <div className="flex items-center justify-between pt-2">
                  <span className="text-base font-semibold">{t('admin.billing.totalInvoiceAmount')}</span>
                  <span className="text-2xl font-bold text-primary">{formatCurrency(invoiceAmount)}</span>
                </div>
              </div>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setIsInvoiceDialogOpen(false); setSelectedSchoolId(''); }}>{t('common.cancel')}</Button>
            <Button onClick={handleGenerateInvoice} disabled={!selectedSchool}>{t('admin.billing.generateInvoice')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
