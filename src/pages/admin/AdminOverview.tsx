...
export default function AdminOverview() {
    const {data: schools, isLoading, error} = useApiQuery(['schools'], '/schools');
    const {t} = useTranslation();

    if (isLoading) return <div>Loading...</div>
    if (error) return <div> {toast.error(error.message || 'Failed to fetch schools')}</div>
    ...
    // re-use when I have more than 6 schools
                ...
                <Card className="border-border/50">
                    ...
                    <CardContent>
                      ...
                      <span className="text-sm font-medium text-muted-foreground">
                        {school.name.split(' ').map((n: never[]) => n[0]).join('').slice(0, 2)}
                      </span>
                                        </div>
                                        <div>
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-medium">{school.studentsCount ? school.studentsCount : 876} {t('common.students')}</p>
                                        <p className={`text-xs ${(school.schoolStatus ? school.schoolStatus : 'Active') === 'Active' ? 'text-success' : 'text-warning'}`}>
                                            {t(`common.${school.schoolStatus.toLowerCase()}`)}
                                        </p>
                                    </div>
                                </div>
                            ))}
